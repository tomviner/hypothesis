...
from .operations import Rot, Drop, Swap, Check, PushRandom, ReadAndWrite, \
    UnaryOperator, BinaryOperator


def operation(*args, **kwargs):
    """Add an operation which pops arguments from each of the varstacks named
    in args, passes the result in that order to function and pushes the result
    of the invocation onto target.

    If target is None the result is ignored.
...
def check(*args, **kwargs):
    ...
    without popping their result and passes them in order to test.

    If test returns something truthy this operation passes, else it will
    fail.
...
def generate(*args, **kwargs):
    """Add a generator for operations which produces values by calling produce
    with a Random instance and pushes them onto target."""
...
def ints(target='ints'):
    ...
    Defines some generators, and adds basic and arithmetic operations to
    target
...
def lists(source, target):
    """Operations which populate target with lists whose elements come from
    source."""
    return (
        basic_operations(target),
